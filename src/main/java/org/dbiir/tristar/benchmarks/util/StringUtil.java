  ...
  public static List<String> splitToList(Pattern delim, String str) {
    ...
    for (String s : arr) {
      ...
      while (left_idx < right_idx
          && Pattern.matches(WHITESPACE.pattern(), s.substring(left_idx, left_idx + 1))) {
        left_idx++;
      }
      while (right_idx > left_idx
          && Pattern.matches(WHITESPACE.pattern(), s.substring(right_idx - 1, right_idx))) {
        right_idx--;
      }
